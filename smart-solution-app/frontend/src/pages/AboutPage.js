    ...
    <div>
      <section style={{ background: `linear-gradient(135deg,${C.navy},${C.navyLight})`, padding: "100px 24px 72px", position: "relative" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", textAlign: "center", position: "relative", zIndex: 1 }}>
          ...
          <h1 style={{ fontFamily: C.FD, fontSize: 72, color: "white", marginBottom: 18 }}>OUR STORY</h1>
          <p style={{ color: "rgba(255,255,255,0.6)", fontSize: 18, maxWidth: 600, margin: "0 auto" }}>Powering South Africa's homes and businesses with expert electrical solutions since 2024.</p>
        </div>
      ...
      <section style={{ padding: "88px 24px", background: "white" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 72, alignItems: "start" }}>
          {/* Founder card */}
          <div style={{ background: `linear-gradient(135deg,${C.navy},${C.navyLight})`, borderRadius: 24, padding: "52px", position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
                {[["10+", "Years"], ["150+", "Solved Electrical Problems"], ["200+", "Happy Clients"]].map(([n, l], i) => (
                  <div key={i} style={{ textAlign: "center", padding: "14px 8px", background: "rgba(255,255,255,0.05)", borderRadius: 12 }}>
                    <div style={{ fontFamily: C.FD, fontSize: 32, color: C.orange }}>{n}</div>
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 28 }}>
            {[
              ...
              <div key={i} style={{ background: m.dark ? C.navy : "white", borderRadius: 20, padding: "48px", borderLeft: `6px solid ${C.orange}`, boxShadow: m.dark ? "0 4px 24px rgba(10,22,40,0.2)" : "0 4px 24px rgba(0,0,0,0.06)" }}>
      ...
      <section style={{ padding: "72px 24px", background: `linear-gradient(135deg,${C.navy},${C.navyLight})` }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", textAlign: "center" }}>
          <h2 style={{ fontFamily: C.FD, fontSize: 52, color: "white", marginBottom: 14 }}>LET'S BUILD SOMETHING GREAT</h2>
          <p style={{ color: "rgba(255,255,255,0.6)", fontSize: 17, marginBottom: 34 }}>Contact us today to discuss your electrical problems then we'll provide a solution.</p>
          <button onClick={() => navigate('/contact')} style={{ background: C.orange, color: "white", border: "none", padding: "13px 38px", borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: "pointer", fontFamily: C.FB }}>Contact Us Today</button>
        </div>
      </section>
    </div>
  );
